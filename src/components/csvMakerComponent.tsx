...
export default function CsvMakerComponent({
  landingTab,
}: {
  ...
  const deleteEligibleParticipant = (
    temporaryId: string,
    csvType: "airdrop" | "poap"
  ) => {
    if (csvType == "airdrop") {
      dispatch(
        setAirdropMakerList(
  ...
  const saveCSV = (csvType: "airdrop" | "poap") => {
    if (csvType == "airdrop") {
      ...
      toast.success(
        "POAP CSV Saved Successfully! Redirecting to create POAP, please wait..."
      );
      setTimeout(() => {
        dispatch(setShowCSVMaker(false));
        ...
        >
          ...
          >
            ...
            >
              ...
              <div className="relative text-center py-1 tabs overflow-hidden">
                {stateTabs.map((ele) => (
                  <button
                    key={ele.name}
                ...
                <div>
                  <div className="flex gap-4 flex-col md:flex-row mb-4">
                    ...
                    <button
                      ...
                      onClick={() => {
                        addEligibleParticipant("airdrop");
                      }}
                    >
                      Add
                    </button>
                  ...
                  <div>
                    <div className="w-full p-2 h-[200px] overflow-y-auto border-2 border-[2px] border-[#FFFFFF17] rounded-md bg-transparent">
                      {poapMakerList.length > 0 &&
                        poapMakerList.map(
                          (eligibleParticipant, index: number) => {
                            return (
                              <div className="flex justify-between items-center">
                                <div className="flex items-center">
                                  <div className="pr-4">{index + 1}.</div>
