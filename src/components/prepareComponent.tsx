...
export function PrepareComponent() {
    ...
    const deleteEligibleParticipant = (temporaryId: string) => {
        setAirdropMakerList(airdropMakerList.filter((eligibleParticipant: IAirdropList) => (eligibleParticipant.id != temporaryId)));
    }
    ...
    if (file) {
      Papa.parse(file, {
                complete: (results: any) => {
                    ...
                    const invalidAddresses = results.data.filter((result: ICSV) => (!(ethers.isAddress(result.address))))
    ...
    return (
        <div className="w-full flex justify-center items-center text-white p-2">
            <div className="p-4 w-full lg:w-[400px] xl:w-[600px] border-[3px] border-[#FFFFFF17] rounded-xl" style={{background: "#8989890D", backdropFilter: "blur(150px)"}}>
                ...
                <button className="w-full bg-[#00A7FF] text-white py-2 rounded-md" onClick={nextPage}>Continue</button>
            </div>
            ...
            <div className={`${showCSVMaker ? "h-screen w-full flex justify-center items-center bg-[transparent] absolute top-[0] left-[0] backdrop-blur-lg p-4": "hidden"}`}>
                <div className="w-full md:w-[600px] border-[3px] border-[#FFFFFF17] p-4 rounded-md flex flex-col gap-4 bg-[#0F195B]">
                    ...
                    <button className="w-full bg-[#00A7FF] text-white py-2 rounded-md" onClick={downloadCSV}>Download</button>
                </div>
            </div>
            {/* CSV Maker ends here */}
        </div>
